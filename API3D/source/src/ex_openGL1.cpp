...
int main(int argc, char **argv) {
  //std::shared_ptr<drawNS::Draw3DAPI> api(new APIopenGL3D(-5,5,-5,5,-5,5,1000,&argc,argv)); //włacza gnuplota, pojawia się scena [-5,5] x [-5,5] x [-5,5] odświeżana co 1000 ms
  drawNS::Draw3DAPI * api = new APIopenGL3D(-5,5,-5,5,-5,5,1000,&argc,argv); //alternatywnie zwykły wskaźnik    
    api->change_ref_time_ms(0); //odświeżanie sceny zmienione na opcję "z każdym pojawieniem się lub zniknięciem kształtu"
    uint a=api->draw_line(drawNS::Point3D(0,0,0),drawNS::Point3D(2,0,0)); //rysuje linię pomiędzy (0,0,0) a (2,0,0), zapamiętuje id kształtu w zmiennej a
    ...
    uint b=api->draw_polygonal_chain(vector<Point3D> {drawNS::Point3D(0,0,0),
                                                     drawNS::Point3D(0,-4,0),drawNS::Point3D(-4,-4,0),
